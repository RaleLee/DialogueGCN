...
def train_or_eval_graph_model(model, loss_function, dataloader, epoch, cuda, optimizer=None, train=False):
    ...
    avg_fscore = round(f1_score(labels, preds, average='macro', labels=[0, 1, 2, 4, 5, 6]) * 100, 2)
    # Add precision and recall
    precision = round(precision_score(labels, preds, average='macro', labels=[0, 1, 2, 4, 5, 6]) * 100, 2)
    recall = round(recall_score(labels, preds, average='macro', labels=[0, 1, 2, 4, 5, 6]) * 100, 2)
