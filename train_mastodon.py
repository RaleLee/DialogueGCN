...
from dataloader import IEMOCAPDataset, DailyDialogueDataset
...
def train_or_eval_graph_model(model, loss_function, dataloader, epoch, cuda, optimizer=None, train=False):
    ...
    # Mastodon doesn't care about '*' label
    avg_loss = round(np.sum(losses)/len(losses), 4)
    avg_accuracy = round(accuracy_score(labels, preds)*100, 2)
    avg_fscore = round(f1_score(labels,preds, average='micro', labels=[0,2])*100, 2)
    # Add precision and recall 
    precision = round(precision_score(labels,preds, average='micro',labels=[0,2])*100, 2)
    recall = round(recall_score(labels, preds, average='micro',labels=[0,2])*100, 2)
...
if __name__ == '__main__':
    ...
    #                                                               batch_size=batch_size,
    #                                                               num_workers=0)
    train_loader, valid_loader, test_loader = get_DailyDialogue_loaders('mastodon/mastodon_bert.pkl',
    ...
    for e in range(n_epochs):
        ...
        if args.graph_model:
            train_loss, train_acc, _, _, train_fscore, _, _, _, _, _, train_precision, train_recall = train_or_eval_graph_model(model, loss_function, train_loader, e, cuda, optimizer, True)
            valid_loss, valid_acc, _, _, valid_fscore, _, _, _, _, _, valid_precision, valid_recall = train_or_eval_graph_model(model, loss_function, valid_loader, e, cuda)
            test_loss, test_acc, test_label, test_pred, test_fscore, _, _, _, _, _, test_precision, test_recall = train_or_eval_graph_model(model, loss_function, test_loader, e, cuda)
        ...
        print('epoch: {}, train_loss: {}, train_acc: {}, train_fscore: {}, train_precision: {}, train_recall: {}, valid_loss: {}, valid_acc: {}, valid_fscore: {}, valid_precision: {}, valid_recall: {}, test_loss: {}, test_acc: {}, test_fscore: {}, test_precision: {}, test_recall: {}, time: {} sec'.\
                format(e+1, train_loss, train_acc, train_fscore, train_precision, train_recall, valid_loss, valid_acc, valid_fscore, valid_precision, valid_recall, test_loss, test_acc, test_fscore, test_precision, test_recall, round(time.time()-start_time, 2)))
