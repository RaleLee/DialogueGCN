...
def train_or_eval_graph_model(model, loss_function, dataloader, epoch, cuda, optimizer=None, train=False):
    ...
    avg_fscore_mi = round(f1_score(labels, preds, average='weighted') * 100, 2)
    # Add precision and recall
    precision_mi = round(precision_score(labels, preds, average='weighted') * 100, 2)
    recall_mi = round(recall_score(labels, preds, average='weighted') * 100, 2)
    print('fscore: {}, precision: {}, recall: {}'.format(avg_fscore_mi, precision_mi, recall_mi))
