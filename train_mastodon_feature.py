...
def get_DailyDialogue_loaders(path, batch_size=32, num_workers=0, pin_memory=False):
    trainset = DailyDialogueDataset3('train', path)
    testset = DailyDialogueDataset3('test', path)
    validset = DailyDialogueDataset3('valid', path)
...
def train_or_eval_graph_model(model, loss_function, dataloader, epoch, cuda, optimizer=None, train=False):
    ...
    avg_fscore = round(f1_score(labels, preds, average='micro', labels=[1, 2]) * 100, 2)
    # Add precision and recall
    precision = round(precision_score(labels, preds, average='micro', labels=[1, 2]) * 100, 2)
    recall = round(recall_score(labels, preds, average='micro', labels=[1, 2]) * 100, 2)
...
if __name__ == '__main__':
    ...
    graph_h = 100
    kernel_sizes = [3, 4, 5]
    glv_pretrained = np.load(open('mastodon/glv_embedding_matrix2', 'rb'))
    ...
    # 0-0-no emotion-0.831
    # 1-5-sadness-0.0112
    # 2-2-disgust-0.0034
    # 3-6-surprise-0.0177
    # 4-3-fear-0.0017
    # 5-4-happiness-0.1251
    # 6-1-anger-0.0099
    ...
    #                                   1/0.127711,
    #                                   1/0.252668])
    loss_weights = torch.FloatTensor([1 / 0.831,
                                      1 / 0.0112,
                                      1 / 0.0034,
                                      1 / 0.0177,
                                      1 / 0.0017,
    ...
    #                                                               num_workers=0)
    if args.class_weight:
        train_loader, valid_loader, test_loader = get_DailyDialogue_loaders('mastodon/mastodon2.pkl',
